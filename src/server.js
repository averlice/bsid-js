...
// --- Log Interceptor (KV Storage) ---
async function addToBuffer(level, args, env) {
    const msg = args.map(arg => typeof arg === 'object' ? JSON.stringify(arg) : String(arg)).join(' ');
    const logEntry = { time: new Date().toISOString(), msg, level };
    
    try {
        if (env && env.LOG_STORAGE) {
            // Get existing logs
            const existingRaw = await env.LOG_STORAGE.get('debug_logs');
            ...
            // Save back to KV
            await env.LOG_STORAGE.put('debug_logs', JSON.stringify(logs));
        }
    } catch (e) {
...
// Wrap console to capture logs, but they now need 'env' which isn't globally available
// We will instead call a log helper inside the request handler.
function logInfo(env, ...args) {
    console.log(...args);
    addToBuffer('info', args, env);
}
function logWarn(env, ...args) {
    console.warn(...args);
    addToBuffer('warn', args, env);
}
function logError(env, ...args) {
    console.error(...args);
    addToBuffer('error', args, env);
}

// Route Admin Requests
router.all('/admin*', (request, env) => {
    logInfo(env, `[Admin] Path: ${new URL(request.url).pathname}`);
...
router.post('/', async (request, env, ctx) => {
  ...
  if (interaction.type === InteractionType.APPLICATION_COMMAND) {
    ...
    logInfo(env, `[Interaction] User: ${username} (${userId}) ran command: /${name}`);
